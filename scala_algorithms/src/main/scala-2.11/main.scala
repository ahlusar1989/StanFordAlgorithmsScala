  ...
  def main(args: Array[String]) {
    ...
    def merge(as: List[Int], bs: List[Int]): List[Int] = {
      ...
      while (i < as.size || j < bs.size) {
        //      if i is equal to the maximum element and j is less than bs.size
    ...
    val input = io.Source.fromFile(new java.io.File("../integers.txt")).getLines.map(_.toInt).toList

    println(sortAndCountSplitInv(input))
  }
}
